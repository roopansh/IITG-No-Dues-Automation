...
<?php
	...
	for($i=0;$i< $count_designtion;$i++)
	{
        ...
        if($temp == $main_low )
        {
        	...
        	$Hostel  = (string)$designation_prof_array[$i][$Hostel];
        	break; 
        }
	}
	echo "<h1>Hostel $Hostel</h1>";

	$student=DB::table('student')->where('Hostel',$Hostel)->where('Care Taker','1')->where($main, '0')->orderBy('Department')->orderBy('Student Name')->get();
	...
	if(isset($_POST['submit']))
	{
		...
		{ 
			...
			if( array_key_exists($temp, $_POST) && $_POST[$temp]) {
				DB::table('student')->where('Hostel',$Hostel)->where('Care Taker','1')->where($main, '0')->orderBy('Department')->orderBy('Student Name')->where('Roll No', $roll)->update([$main_Comments => $_POST[$temp]]);
			}

			$temp  = 'cb_'.$i.'a'.$roll;
			if (array_key_exists($temp, $_POST) && $_POST["$temp"] == 'on')
			{
				DB::table('student')->where('Hostel',$Hostel)->where('Care Taker','1')->where($main, '0')->orderBy('Department')->orderBy('Student Name')->where('Roll No', $roll)->update([$main => true]);
			}			
...
<?php
	...
	for ($i=0; $i < $student_Count; $i++) { 
		...
		$prevcom = DB::table('student')->where('Student Name' , $tempname)->where('Hostel',$Hostel)->where('Care Taker','1')->value($main_Comments);
