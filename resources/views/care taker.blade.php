...
<?php
	...
	$student=DB::table('student')->where('Hostel',$Hostel)->where($main, '0')->orderBy('Department')->orderBy('Student Name')->get();
	...
	if(isset($_POST['submit']))
	{
		...
		{ 
			...
			if( array_key_exists($temp, $_POST) && $_POST[$temp]) {
				DB::table('student')->where('Hostel',$Hostel)->where($main, '0')->orderBy('Department')->orderBy('Student Name')->where('Roll No', $roll)->update([$main_Comments => $_POST[$temp]]);
			}

			$temp  = 'cb_'.$i.'a'.$roll;
			if (array_key_exists($temp, $_POST) && $_POST["$temp"] == 'on')
			{
				DB::table('student')->where('Hostel',$Hostel)->where($main, '0')->orderBy('Department')->orderBy('Student Name')->where('Roll No', $roll)->update([$main => true]);
			}			
