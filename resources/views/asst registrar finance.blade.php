...
<?php
	...
	$main="Asst Registrar Finance";
	$main=(string)$main;

	$main_low="asst registrar finance";
	...
	echo "<h1>Assistant Registrar Finance</h1>";

	$student=DB::table('student')->where('Head Department','1')->where($main, '0')->orderBy('Department')->orderBy('Student Name')->get();
	...
	if(isset($_POST['submit']))
	{
		...
		{ 
			...
			if( array_key_exists($temp, $_POST) && $_POST[$temp]) {
				DB::table('student')->where('Head Department','1')->where($main, '0')->orderBy('Department')->orderBy('Student Name')->where('Roll No', $roll)->update([$main_Comments => $_POST[$temp]]);
			}

			$temp  = 'cb_'.$i.'a'.$roll;
			if (array_key_exists($temp, $_POST) && $_POST["$temp"] == 'on')
			{
				DB::table('student')->where('Head Department','1')->where($main, '0')->orderBy('Department')->orderBy('Student Name')->where('Roll No', $roll)->update([$main => true]);
			}			
...
<?php
	...
	for ($i=0; $i < $student_Count; $i++) { 
		...
		$temp  = 'com_'.$i.'a'.$roll;
		$prevcom = DB::table('student')->where('Head Department','1')->value($main_Comments);
